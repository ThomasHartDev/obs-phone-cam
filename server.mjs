...
// Command Center (and other laptop UIs) fetch QR / lan.json / control cross-origin.
const CORS = {
...
async function handleRequest(req, res) {
  ...
  // HTTP control path for Command Center (browser cannot open wss://localhost
  // from a public HTTPS origin — Chrome Private Network Access). Server-side
  // CC proxies POST here over Tailscale.
...
// laptop "controller" sockets and multi "viewer" sockets (CC live preview).
...
function handleWs(ws, req) {
  ...
  // Extra watchers (Command Center film tab). Many OK; never kick OBS.
  if (role === "viewer") {
